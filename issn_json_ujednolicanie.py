...
plik=r"C:\Users\darek\zagubiony_arto.mrk.mrk"
...
new.to_excel("zagubionyArto.xlsx", sheet_name='Sheet_name_1')  
...
### Ujednolicanie MARC
df_ujednolicone=pd.read_excel(r"C:\Users\darek\zagubionyArto.xlsx",sheet_name=0)
dict_pole100_viaf=dict(zip(df_ujednolicone['773'], df_ujednolicone.ujednolicone)) 
patternISSN=r'(?<=\$x)\d{4}-[a-zA-Z0-9_.-]{4}'
patterntitle=r'(?<=\$t).*?(?=\$|$)' 
cale=[]
for rekord in tqdm(dictrec):
