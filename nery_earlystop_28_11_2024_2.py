...
def prepare_data(data, tokenizer, tag2id, max_length=256):
    ...
    for text, annotation in data:
        ...
        label_ids = [
            tag2id.get(lbl, tag2id['O']) if lbl != 'O' else tag2id['O'] 
            for lbl in sequence_labels
        ]
        
        # Dla miejsc, gdzie label_ids jest -100, pozostaw -100
        label_ids = [
            label_ids[i] if attention_mask[i] == 1 and label_ids[i] != tag2id['O'] else -100
...
# Przykładowa lista entity_stats


# Funkcja przygotowująca dane (zakładam, że jest zdefiniowana)
# def prepare_data(...): ...
...
# Konwersja TensorDataset do słowników
def convert_to_dict(input_ids, attention_masks, labels):
    return {
...
train_dict = convert_to_dict(input_ids, attention_masks, labels)
eval_dict = convert_to_dict(input_ids_eval, attention_masks_eval, labels_eval)
...
for i in range(1):  # Sprawdź pierwsze 3 przykłady
    original_text = transformed_data_train[i][0]  # Pobranie oryginalnego tekstu
