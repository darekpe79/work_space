# -*- coding: utf-8 -*-
"""
Created on Fri Aug 18 21:28:23 2023
...
def convert_marc_to_mrk(mrc_file_path, mrk_file_path):
    ...
    # Set the progress bar maximum for counting phase
    ...
    with open(mrc_file_path, 'rb') as data:
        ...
        for _ in reader:
            records_counted += 1
            progress_bar["value"] = (20 * records_counted) / 400000
            root.update_idletasks()  # Update the GUI as we count

    # Now adjust the progress bar for the conversion phase
    total_records = records_counted + 20  # We add 20 to account for the counting phase
    ...
    with open(mrc_file_path, 'rb') as data:
        ...
        for record in reader:
            ...
            root.update_idletasks()  # Update the GUI during conversion
