...
with open('D:/Nowa_praca/słowniki_praca_fin_cze_sp_pl/27072023cze_broader_narrower_labels.json', encoding='utf-8') as user_file:
...
# fin_df.to_excel("label-broader_yso-narrower.xlsx")
...
for key,val in narrower_elb_id.items():
    if key not in narrower_fi_cze:
       ...
       unique(value)                 
       narrower_fi_cze[narrow_excel]=value
fin_df2=pd.DataFrame.from_dict(narrower_fi_cze, orient='index')  
fin_df2.to_excel("key-narrower_val-broader_cze_fin.xlsx")   
#klucz broader                
broader_fi_cze={}
...
for key,val in broader_elb_id.items():
    if key not in broader_fi_cze:
       ...
       unique(value)                 
       broader_fi_cze[broader_excel]=value             
                
fin_df2=pd.DataFrame.from_dict(broader_fi_cze, orient='index')  
fin_df2.to_excel("key-broader_val-narrower_cze_fin.xlsx")           
...
#klucz broader wartosć narrower poprawa tabelki:
    
key_broader_val_narrower=pd.read_excel('D:/Nowa_praca/słowniki_praca_fin_cze_sp_pl/28072023key-broader_val-narrower_cze_fin.xlsx', sheet_name='Sheet1',dtype=str)
dict_from_df = key_broader_val_narrower.set_index(key_broader_val_narrower.columns[0]).T.to_dict('list')
new_dict = {key: {'narrower': [i for i in value if pd.notnull(i)]} for key, value in dict_from_df.items()}
fin_df=pd.DataFrame.from_dict(new_dict, orient='index')

fin_df.to_excel("key-broader_val-narrower_cze_fin_ver2.xlsx")           
