...
HARD_CAP       = 768  # maks. kontekst wg Twojej polityki
MODEL_LIMIT    = int(getattr(model.config, "max_position_embeddings", 131072))
BIG_NUMBER     = 2_000_000_000  # wart. wartownicza na „nieskończony” limit w tokenizerach
...
# pre-encode stałych
IDS_TRUE  = tokenizer.encode("True",  add_special_tokens=False)
IDS_FALSE = tokenizer.encode("False", add_special_tokens=False)
TRUE_ID0  = IDS_TRUE[0]
FALSE_ID0 = IDS_FALSE[0]
...
def tokenize_row_dynamic_budgeted(example):
    ...
    # 3) Szybkie pre-cięcie po znakach (żeby tokenizer nie generował setek tysięcy ID)
...
# ========================= 6) METRYKI (pierwszy token odpowiedzi) =========================

def compute_metrics(eval_pred):
    logits, labels = eval_pred  # logits: [B, L, V]; labels: [B, L]
    logits = torch.tensor(logits)
    labels = torch.tensor(labels)

    preds_bin, labels_bin = [], []
    for i in range(labels.shape[0]):
        ans_idx = (labels[i] != IGNORE_INDEX).nonzero(as_tuple=False)
        if len(ans_idx) == 0:
            continue
        pos = ans_idx[0].item()
        logp = torch.log_softmax(logits[i, pos, :], dim=-1)
        pred = "True" if logp[TRUE_ID0] > logp[FALSE_ID0] else "False"
        true_token_id = labels[i, pos].item()
        true_label = "True" if true_token_id == TRUE_ID0 else "False"
        preds_bin.append(pred)
        labels_bin.append(true_label)

    acc = accuracy_score(labels_bin, preds_bin) if labels_bin else 0.0
    precision, recall, f1, _ = precision_recall_fscore_support(
        labels_bin, preds_bin, average='weighted', zero_division=0
    )
...
def preprocess_logits_for_metrics(logits, labels):
    ...
    with torch.no_grad():
        if labels is None:
            ...
            # pozycja pierwszego nie-IGNORE_INDEX w każdej sekwencji = start odpowiedzi
            pos_list = []
            for i in range(labels.shape[0]):
        ...
        # zwracamy tylko logity dla 2 tokenów klas:
        return sel[:, [TRUE_ID0, FALSE_ID0]]

# --- metryki dla zredukowanych logitów (N, 2) ---
def compute_metrics_2class(eval_pred):
    ...
    # pred: 0 -> "True", 1 -> "False"
    preds = two_logits.argmax(axis=-1)
...
eval_dataset_small = eval_dataset.select(range(min(len(eval_dataset), 1000)))
# i podmień w Trainer:
...
training_args = TrainingArguments(
    ...
    # logi i walidacja/zapis co K kroków
    logging_steps=50,
    eval_strategy="steps",
    eval_steps=200,
    save_strategy="steps",
...
trainer = Trainer(
    ...
    tokenizer=tokenizer,              # ok (ostrzeżenie deprec. można zignorować)
    data_collator=data_collator,
    compute_metrics=compute_metrics_2class,
    preprocess_logits_for_metrics=preprocess_logits_for_metrics,
    callbacks=[
        EarlyStoppingCallback(
            early_stopping_patience=3,      # 3 × eval bez poprawy -> stop
            early_stopping_threshold=5e-4   # minimalna sensowna poprawa F1
        )
    ],
)
